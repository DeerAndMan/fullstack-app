...
| 运算符 | 描述                                             | 实例                                         |
| ------ | ------------------------------------------------ | -------------------------------------------- |
| &      | 返回变量存储地址，取 “址” 符。获取变量的内存地址 | &a，将给出变量的实际地址。获取当前变量的地址 |
| *      | 指针变量，取 “值” 符。指向的原始值               | *a，是一个指针变量。访问地址指向的值         |
...
+ select
  ...
  + select 可以同时监控多个通道的读写操作，程序会阻塞在 select 语句处，直到其中一个 case 中的通道操作可以执行（非阻塞状态）
  ...
  + ```go
    ...
    case channel3 <= value:
     	// 执行代码
    default: // 默认情况下，select会阻塞，直到有通道可以接收数据。
      // 所有通道都没有准备好，执行的代码
    }
    ```
...
![defer 执行时机](./assets/defer 执行时机.png)
...
![slice 长度 容量的本质](./assets/slice 长度 容量的本质.png)
...
make 主要用于创建**切片（slice）映射（map）通道（channel）**这三种**引用类型**的数据结构，并为其分配内存和进行初始化。并返回初始化后的**值**类型本身

```go
// 初始化长度为 3 的切片
slice := make([]int, 3) // len=5 cap=5 输出：slice: [0 0 0 0 0]


m1 := mack(map[string]int, 100) // 预分配约 100 个键的空间
```

### new 函数

用于创建**指定类型**的零值变量，并返回该变量的指针。

```go
// 使用 new() 创建一个 int 类型的零值指针变量
numPtr := new(int) // 输出：numPtr: 0
```
...
| 运算符 | 描述                         | 实例                                             |
| ------ | ---------------------------- | ------------------------------------------------ |
| &      | 返回变量存储地址，取 “址” 符 | &a，将给出变量的实际地址。获取当前变量的**地址** |
| *      | 指针变量，取 “值” 符         | *a，是一个指针变量。访问地址指向的**值**         |
|        | 数据类型                     | *指向的类型                                      |
...
```go
// 适用于处理严重的、不可恢复但希望避免程序崩溃的错误
Panic("抛出一个错误提示！！！")

// 捕获错误方法
defer func() {
		if r := recover(); r!= nil {
    		fmt.Printf("中间层捕获到 panic，异常消息: %v\n", r)
    }
}()
...
```go 
go mod tidy // 主要作用是清理和整理项目的依赖关系，确保 go.mod 和 go.sum 文件与项目实际使用的依赖保持精确一致。
```
